...
def main(config):
    ...
    if config['norm_from']:
        ...
        # Explicitly copy the DataFrame slice
        train_features = my_data.feature_df.loc[train_indices].copy()
        normalized_train_features = normalizer.normalize(train_features)
        my_data.feature_df.loc[train_indices] = normalized_train_features
    ...
    if normalizer is not None:
        ...
        if len(val_indices):
            val_features = val_data.feature_df.loc[val_indices].copy()  # Create a copy of the data slice
            normalized_val_features = normalizer.normalize(val_features)
            val_data.feature_df.loc[val_indices] = normalized_val_features  # Safely replace the original data

        # Normalize test data
        if len(test_indices):
            test_features = test_data.feature_df.loc[test_indices].copy()  # Create a copy of the data slice
            normalized_test_features = normalizer.normalize(test_features)
            test_data.feature_df.loc[test_indices] = normalized_test_features  # Safely replace the original data
